...
from src.tool.tool_auto import *
...
def train(model,
    ...
    validation_steps = get_steps_per_epoch(validation_path)

    # example  all= 22011
    #batch size = 16 batch size 在tool/data_prepare下
    #steps_per_epoch = all / batch size=22011/16=1376
    steps_per_epoch=get_steps_per_epoch(train_path)
   ...
   # test_datas, test_lables = get_test_all(test_path='../deal-data/test/',image_size=224,channel=3)
    generate_train = generate_batch_data(train_path)


    generate_validation = generate_batch_data(validation_path)
    ...
    '''
       ...
       -------------------------------------------------------------------------------------
    '''
    dirname = train_process_path+'/' + str(train_id) + '/'
    ...
    '''
       ...
       -------------------------------------------------------------------------------------
    '''
    my_spatial_model.compile(loss=loss_fuction, optimizer=optimizer, metrics=['accuracy'])
    ...
    my_spatial_model.fit_generator(
            ...
            callbacks = [reduce_lr,checkpointer,stopping,history],
            shuffle=True,
